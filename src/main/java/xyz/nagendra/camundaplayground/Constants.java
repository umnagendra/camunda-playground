package xyz.nagendra.camundaplayground;

public class Constants {

    private Constants() {}

    public static final String VAR_NAME_STARTED_BY = "startedBy";
    public static final String VAR_NAME_STARTED_AT = "startedAt";
    public static final String VAR_NAME_NUM = "num";
    public static final String VAR_NAME_IS_EVEN = "isEven";
    public static final String VAR_NAME_APPROVED_BY = "approvedBy";
    public static final String VAR_NAME_ERROR_OCCURED = "ERROR-OCCURED";
    public static final String VAR_NAME_ERROR_MSG = "ERROR-MSG";
    public static final String VAR_NAME_STOCK_QUOTES_LIST = "stockQuotesList";

    public static final String TASK_ID_WAIT_APPROVAL = "Task_Wait_Approval";
}
